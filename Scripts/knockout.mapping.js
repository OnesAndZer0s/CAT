...
(function(d) {
    ...
    function w(b, c) {
        for (var a in c)
            if (c.hasOwnProperty(a) && c[a])
                if (a && b[a] && "array" !== e.getType(b[a])) w(b[a], c[a]);
                else if ("array" === e.getType(b[a]) && "array" === e.getType(c[a])) {
            for (var d = b, g = a, i = b[a], r = c[a], t = {}, h = i.length - 1; 0 <= h; --h) t[i[h]] = i[h];
            for (h = r.length - 1; 0 <= h; --h) t[r[h]] =
                r[h];
            i = [];
            r = void 0;
    ...
    function p(b, c) {
        "array" !==
        e.getType(b) && (b = "undefined" === e.getType(b) ? [] : [b]);
        "array" !== e.getType(c) && (c = "undefined" === e.getType(c) ? [] : [c]);
        return d.utils.arrayGetDistinctValues(b.concat(c))
    }

    function E(b, c, a, f, g, i, r) {
        var t = "array" === e.getType(d.utils.unwrapObservable(c)),
            i = i || "";
        if (e.isMapped(b)) var h = d.utils.unwrapObservable(b)[q],
            a = D(h, a);
        var m = function() {
            ...
            j = function(b) {
                ...
                d.dependentObservable = function(a, b, c) {
                    ...
                    var f = c.deferEvaluation,
                        L = !1;
                    c.deferEvaluation = !0;
                    a = new G(a, b, c);
                    if (!f) {
                        var g = a,
                            f = d.dependentObservable;
                        d.dependentObservable = G;
                        a = d.isWriteableObservable(g);
                        d.dependentObservable = f;
                        a = G({
                            read: function() {
                                L || (d.utils.arrayRemoveItem(e, g), L = !0);
                ...
                b = d.utils.unwrapObservable(g) instanceof Array ? a[f].create({
                    data: b || c,
                    parent: r,
                    skip: M
                }) : a[f].create({
        ...
        if (t) {
            ...
            d.isObservable(b) || (b = d.observableArray([]), b.mappedRemove = function(a) {
                ...
                function(a) {
                    if (b.mappedIndexOf(a) !== -1) throw Error("There already is an object with the key that you specified.");
                    var c = m() ? j(a) : a;
                    if (x()) {
                        a = y(c, a);
            ...
            var h = B(d.utils.unwrapObservable(b), k).sort(),
                n = B(c, k);
            s && n.sort();
            var s = d.utils.compareArrays(h, n),
                h = {},
                p, z = d.utils.unwrapObservable(c),
                v = {},
                w = !0,
                n = 0;
            ...
            b(z);
            a[f] && a[f].arrayChanged && d.utils.arrayForEach(t,
                function(b) {
            ...
            P(c, function(f) {
                ...
                if (-1 == d.utils.arrayIndexOf(a.ignore, e))
                    if (-1 != d.utils.arrayIndexOf(a.copy, e)) b[f] = c[f];
                    else {
                        var g = H.get(c[f]),
                            h = E(b[f], c[f], a, f, b, e, b),
                            g = g || h;
                        if (d.isWriteableObservable(b[f])) b[f](d.utils.unwrapObservable(g));
                        else b[f] = g;
                        a.mappedProperties[e] = !0
                    }
            })
        } else switch (e.getType(c)) {
            case "function":
                x() ?
                    d.isWriteableObservable(c) ? (c(y(c)), b = c) : b = y(c) : b = c;
                break;
            default:
                if (d.isWriteableObservable(b)) return l = x() ? y(b) : d.utils.unwrapObservable(c), b(l), l;
    ...
    var q = "__ko_mapping__",
        G = d.dependentObservable,
        J = 0,
        F, H, K = ["create", "update", "key", "arrayChanged"],
        M = {},
        v = {
            include: ["_destroy"],
            ignore: [],
    ...
    e.toJS = function(b, c) {
        ...
        if ("array" !== e.getType(j.ignore)) throw Error("ko.mapping.defaultOptions().ignore should be an array.");
        if ("array" !== e.getType(j.include)) throw Error("ko.mapping.defaultOptions().include should be an array.");
        if ("array" !== e.getType(j.copy)) throw Error("ko.mapping.defaultOptions().copy should be an array.");
        c = A(c, b[q]);
    ...
    e.getType = function(b) {
        if (b && "object" === typeof b) {
            ...
            if ("[object Array]" === Object.prototype.toString.call(b)) return "array"
        }
        return typeof b
    };
    e.visitModel = function(b, c, a) {
        ...
        if (O(g)) a = A(a, g[q]), c(b, a.parentName), f = "array" === e.getType(g) ? [] : {};
        ...
        P(g, function(b) {
            if (!(a.ignore && -1 != d.utils.arrayIndexOf(a.ignore, b))) {
                ...
                "array" === e.getType(g) ? i &&
                    (m += "[" + b + "]") : (i && (m += "."), m += b);
                h.parentName = m;
                if (!(-1 === d.utils.arrayIndexOf(a.copy, b) && -1 === d.utils.arrayIndexOf(a.include, b) && g[q] && g[q].mappedProperties && !g[q].mappedProperties[b] && "array" !== e.getType(g))) switch (e.getType(d.utils.unwrapObservable(j))) {
                    case "object":
                    case "array":
                    case "undefined":
